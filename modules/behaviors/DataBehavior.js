(function(root, factory) {
   ...
   *  * __view.getBehavior('thisBehaviorAlias').toJSON()__ - Either array of data if returnSingleResult is false (default),
   *                                                        Or the object data directly if returnSingleResult is true.
   *  * __view.getBehavior('thisBehaviorAlias').toJSON('some.data.property')__ - returns just that property of the result or undefined if it hasn't been fetched or doesn't exist.
   ...
   *
   *   The context generated by `view.prepare()` will contain all of the data associated with this view namespaced by the behavior's alias on the view.
   *   This the same data as returned by `view.getBehavior('thisBehaviorAlias').toJSON()`.
   *
   *     View.extend({
   *       behaviors: {
   *         thisBehaviorAlias: {
   ...
   *     {
   *       ...,
   *       thisBehaviorAlias: <result of view.getBehavior('thisBehaviorAlias').toJSON()>
   *     }
   *
  ...
  var DataBehavior = Behavior.extend({
     ...
     * @param [behaviorOptions.id=behaviorOptions.ids] {String|Number|String[]|Number[]|Object|Function} - alias for ids.  id or ids is required.
     * @param [behaviorOptions.ids=behaviorOptions.id] {String|Number|String[]|Number[]|Object|Function} - duck-typed property that identifies the ids to use. id or ids is required.
     *   - {String|Number} - the id to use directly (equivalent to an array of a single id).
     *   - {String[]|Number[]} - the ids to use directly.
     *   - {Object} - more complex configuration that identifies a model-like object that fires a change event and the
     *                property on that object to use. The object can fire the change event for the given property
