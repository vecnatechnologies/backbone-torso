...
describe('A List View', function() {
  ...
  it('can have child views added via a collection', function() {
    ...
    expect(myListView.getChildView(model)).toBeDefined();
    ...
    expect(myListView.getChildView(model).model.cid).toBe(model.cid);
    var model2 = new Model();
    myCollection.add(model2);
    expect(myListView.modelsToRender().length).toBe(2);
    expect(myListView.getChildView(model2)).toBeDefined();
    ...
    expect(myListView.getChildView(model2).model.cid).toBe(model2.cid);
    expect($(_.first(myListView.$el.children())).html()).toBe(myListView.getChildView(model).$el.html());
    expect($(_.first(myListView.$el.children())).is(myListView.getChildView(model).$el)).toBe(true);
    expect(_.first(myListView.$el.children())).toBe(myListView.getChildView(model).el);
    expect(_.last(myListView.$el.children())).toBe(myListView.getChildView(model2).el);
  });
  ...
  it('can have a child view removed via a collection', function() {
    ...
    var model2 = new Model();
    myCollection.add(model2);
    var itemView2 = myListView.getChildView(model2);
  ...
  it('can render child views with specialized prepare content', function() {
    ...
    var model = new Model();
    myCollection.add(model);
    expect(myListView.getChildView(model).context.data).toBe(4);
    myListView = new MyListView({
      collection: myCollection,
      childModel: 'item',
      childView: ItemView,
  ...
  it('can trigger an event when a child view is removed', function(done) {
    var model = new Model();
    myCollection.add(model);
    var childView = myListView.getChildView(model);
  ...
  it('can trigger an event when a child view is added', function(done) {
    ...
    myListView.on('child-view-added', function(data) {
      expect(data.model.cid).toBe(model.cid);
      expect(data.view.cid).toBe(myListView.getChildView(model).cid);
      done();
    });
    myCollection.add(model);
  });
  ...
  it('can reorder child views when the collection order changes', function() {
    ...
    expect(_.first(myListView.$el.children())).toBe(myListView.getChildView(model).el);
    expect(_.last(myListView.$el.children())).toBe(myListView.getChildView(model3).el);
    myCollection.comparator = 'order';
    myCollection.sort();
    expect(_.first(myListView.$el.children())).toBe(myListView.getChildView(model3).el);
    expect(_.last(myListView.$el.children())).toBe(myListView.getChildView(model).el);
  });
  ...
  it('can sort a large list in a reasonable time', function() {
    ...
    myListView._createChildViews();
    myListView.render();
    myCollection.comparator = 'order';
    startTime = new Date().getTime();
    myCollection.sort();
    endTime = new Date().getTime() - startTime;
    expect(myListView.getChildView(myCollection.at(0)).el).toBe(_.first(myListView.$el.find('div.item')));
  ...
  it('can add view to a large list in a reasonable time', function() {
    ...
    expect(myListView.getChildView(newModel)).toBeDefined();
    endTime = new Date().getTime() - startTime;
  ...
  xit('can keep position of carat', function() {
    ...
    var model = new Model();
    myCollection.add(model);
    var view = myListView.getChildView(model);
  ...
  it('has children views that it can add and have them still react to DOM events', function() {
    var model = new Model();
    myCollection.add(model);
    var itemView = myListView.getChildView(model);
