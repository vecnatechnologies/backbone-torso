...
A View should only have to manage its own event callbacks, not those of other objects. Adding an "on" callback is changing an attribute of that model or collection and is more difficult to manage later. Use: `this.listenTo()`, not `this.model.on()`.
