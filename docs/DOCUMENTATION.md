...
A form model provides two methods: ```addMapping``` and ```addTrackedModel```. The method ```addMapping``` allows you to have the form model track some or all of that model's fields. The form model can pull the object model's fields at any time and can even ```startUpdating()``` which will keep the form model up-to-date in real time. Live updating can be useful if the form is in a “read-only” state – you don't usually want the form updating while a user is using it. A mapping can also change, combine, split fields across one or more object models as they enter the form model. Likewise when the form model pushes back to the object model(s), a reverse transform can take place. This is useful if you'd like to go from dollars to cents, or 3-field SSN to one, etc.

You can define a mapping first and add the models you will track later. This is helpful when you want to define the mappings on the class level. You can define a field ```mapping``` on the form model level instead of having to invoke ```addMapping``` on a form model instance. You can add tracked models at any time by calling ```addTrackedModel```. Tracked models and mappings are bound through aliases - basically names. A mapping refers to a model by name and you set the model later using that same name. Multiple mappings can reference the same model using its name.
